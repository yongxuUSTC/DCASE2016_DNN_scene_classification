...
#include "/usr/local/cuda/include/cuda_runtime.h"
#include "/usr/local/cuda/include/cublas_v2.h"
#include "/usr/local/cuda/include/curand.h" //这里可能提示cuda_runtime.h找不到，就要进入curand.h，把cuda_runtime.h的全路径贴上去
//#include <curand.h>
//#include "/usr/local/cuda/include/cuda_runtime.h"
//#include "/usr/local/cuda/include/cublas_v2.h"
...
#define MAXCACHEFRAME 149900

struct BP_WorkSpace
{
	float *in; 								/// input data
	float *out;								/// Output data，输出是float型，它通过softmax变成0,1
	//float *out1;								/// mph183输出的结果，需要用softmax对它进行单独作用
	//float *in1;								/// mph183输出的结果，softmax之前的
	//float *out2;								/// mph183输出的结果，需要用softmax对它进行单独作用
	//float *in2;								/// mph183输出的结果，softmax之前的
...
public:
	...
	~BP_GPU();
public:
	void train(int n_frames, float* in, const int *targ);
	...
	void train_bunch_single(int n_frames, float* in, const int *targ);
  ...
  //float  CrossValid3(int n_frames, const float* in, const float *targ);
