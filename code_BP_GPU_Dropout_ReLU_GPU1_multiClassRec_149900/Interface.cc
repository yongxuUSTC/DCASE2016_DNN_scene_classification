...
void Interface::Initial(int argc, char **argv)
{
	...
	if(para->fea_dim * para->fea_context != para->layersizes[0])//noNAT
	...
	para->targ 	= new int[para->traincache];//targ只分配一个整形？如何和183个输出节点对应起来？？？而我现在貌似只能用float型来代替int型，和clean feature混合在一起用
//	para->targ 	= new float[para->layersizes[numlayers-1] * para->traincache];///////////////////////////////////////////////////////////by yongxu
//para->targ 	= new float[(257*2+1) * para->traincache];///////////////////////////////////////////////////////////by yongxu  //mph183
...
//		sizePerFrame = sizeof(float)*(2+ 257*2+1);/////////////////////////////////////////////////////////////我的输出始终是out_dims维，by yongxu  //mph183
//	offset = tmpframenum * (long int)sizePerFrame + PFILE_HEADER_SIZE;
//	read_tail(fp_targ, offset, tmpsentnum, tmpframepersent);  /// get frame per sent
...
//      i=i-1;
//      for(k=para->fea_dim;k< 2*(para->fea_dim);k++){
//					para->indata[sample_index[cur_sample]* para->layersizes[0] +k +i *para->fea_dim] = (dataori[(frames_processed + 0) *(2+para->fea_dim) +(k-para->fea_dim)+2]+dataori[(frames_processed + 1) *(2+para->fea_dim) +(k-para->fea_dim)+2]+dataori[(frames_processed + 2) *(2+para->fea_dim) +(k-para->fea_dim)+2]+dataori[(frames_processed + 3) *(2+para->fea_dim) +(k-para->fea_dim)+2]+dataori[(frames_processed + 4) *(2+para->fea_dim) +(k-para->fea_dim)+2]+dataori[(frames_processed + 5) *(2+para->fea_dim) +(k-para->fea_dim)+2])/6.0f;
//				}
	...
	//size_perframe = ((257*2+1) +2) *sizeof(float);/////////////////////////by yongxu,我的输出始终是out_dims维float数据 //mph183
	...
	//targori = new float[frames_need_read *(257*2+1 +2)];///////////////by yongxu, 我输出的始终是out_dims维float数据 // +monophone183
...
//			swap32((int *) (&targori[2 +i*(1 +2)]));//mph183
//			//targori[2+j +i*(para->layersizes[numlayers-1] +2)] -= mean[j];
//	//		targori[2+j +i*((257*2+1) +2)] -= mean[j];//mph183
//			//targori[2+j +i*(para->layersizes[numlayers-1] +2)] *= dVar[j];
//		//	targori[2+j +i*((257*2+1) +2)] *= dVar[j];//mph183
//			//printf("speech: targori[2+j +i*((257*2+1) +2)]=%f,\n",targori[2+j +i*((257*2+1) +2)]);						
//		}		
//		//printf("\n");
//	}
...
//			for(k=0;k< (257*2+1);k++){//mph183
//			para->targ[sample_index[cur_sample]* (257*2+1) +k ] = targori[(frames_processed +j+ para->targ_offset) *(2+(257*2+1)) +k+2];	  
//			  }
			cur_sample++;
		}
...
//				}
	...
	//size_perframe = ((257*2+1) +2) *sizeof(float);//////////////////////////////////////////////////////////by yongxu //mph183
	offset = PFILE_HEADER_SIZE + cv_chunk_frame_st[chunk_index]* (long int) size_perframe;
	...
	//targori = new float[frames_need_read *((257*2+1) +2)];/////////////////////////////////////////////////////by yongxu  //mph183
	//cudaMallocHost((void**)&(targori), frames_need_read *(1 +2) * sizeof(int));
	fread((char *)targori,size_perframe,frames_need_read,fp_targ);
	//swap32( &targori[0]);
	swap32( (int*)&targori[0]);
