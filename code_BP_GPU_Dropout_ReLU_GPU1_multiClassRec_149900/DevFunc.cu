...
//__global__ void  kernDevMergeCUDAout(int rows, int cols1, int cols2, float* out_vec1, float* out_vec2, float* out_vec)
//{
//    int row = (blockIdx.x * blockDim.x) + threadIdx.x;
//    if (row < rows)
//    {
...
//		 int j;
//	 	 for(j =0; j< cols1;j++)
//		 	out_vec[(cols1+cols2) *row +j] = out_vec1[cols1 *row +j];
//		 	
//		 for(j =cols1; j< cols1+cols2; j++)
//		 	out_vec[(cols1+cols2) *row +j] = out_vec2[cols2 *row +(j-cols1)];
//		 	
//    }
//}

//__global__ void  kernDevSplitOut(int rows, int cols1, int cols2, float* in_vec, float* out_vec1, float* out_vec2)
//{
//    int row = (blockIdx.x * blockDim.x) + threadIdx.x;
//    if (row < rows)
//    {
...
//		 int j;
//	 	 for(j =0; j< cols1;j++)
//		 	out_vec1[(cols1) *row +j] = in_vec[(cols1+cols2) *row +j];
//		 	
//		 for(j =cols1; j< cols1+cols2; j++)
//		 	out_vec2[(cols2) *row +(j-cols1)] = in_vec[(cols1+cols2) *row +j];
//		 	
//    }
//}
...
//	 	 float energy_s=0.0f;
//	 	 float energy_n=0.0f;
//	 	 //float energy_mfcc=0.0f;
//	 	 
//	 	 	 	for(j =0; j< 257;j++)
//		 	{
//		 energy_s= energy_s + (in_clean[(257*2+1) *i +j])*(in_clean[(257*2+1) *i +j]);
//		 }
//		 energy_s=energy_s/257.0f;
//		 
//		 	  for(j =257; j< 257*2;j++)
//		 	{
//		 energy_n= energy_n + (in_clean[(257*2+1) *i +j])*(in_clean[(257*2+1) *i +j]);
//		 }
//		 energy_n=energy_n/257.0f;
//		 
//		 
//	 	 for(j =0; j< 257;j++)
...
//		 res_vec[cols *i + j] = (2.0f/rows)*0.8f*(in_vec1[cols *i +j]-in_clean[(257*2+1)*i +j]);
//		 res_vec[cols *i + j] = (res_vec[cols *i + j]/energy_s);
...
//		 //int ind = in_index[i];
//		 //res_vec[cols *i + ind] = in_vec1[cols *i +ind] - 1.0f;
//		 res_vec[cols *i + j] = (2.0f/rows)*0.2f*(in_vec1[cols *i +j]-in_clean[(257*2+1)*i +j]);
//		 res_vec[cols *i + j] = (res_vec[cols *i + j]/energy_n);
//		 //res_vec[cols *i + j] = (energy_s/energy_n)*res_vec[cols *i + j];
//		 //res_vec[cols *i + j] = 2.0f*(in_vec1[cols *i +j]-in_clean[cols *i +j]);
//		 //printf("in kernSubClean, res_vec=%f ",res_vec[cols *i + j]);
//		 }
//		 
////		 for(j =257*2; j< 257*2+183;j++)//have 183 dims
////		 {
////		 res_vec[cols *i +j] = in_vec1[cols *i +j]*0.4f;
////		 }
////		 ind = (int) in_clean[(257*2+1) *i + (257*2+0)];//the i-th frame Label
////		 res_vec[cols *i + 257*2 + ind] = in_vec1[cols *i + 257*2 + ind] - 1.0f;
//
//
////////MMSE to replace CE
//		 for(j =257*2; j< 257*2+183;j++)//have 183 dims
